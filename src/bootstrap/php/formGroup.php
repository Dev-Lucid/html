...
class BootstrapFormGroup extends BaseTag
{
    public $tag             = 'div';
    public $label           = null;
    public $help            = null;
    ...
    public function preRender()
    {
        ...
        $label = \DevLucid\html::label()->add($this->label)->for($this->children[0]->name());

        # build the help text area if necessary
        $help = '';

        if (is_null($this->help) === false && $this->help != '') {
            $help = \DevLucid\html::small()->addClass('text-muted')->add($this->help)->render();
        }
    ...
    {
        \DevLucid\html::errorBoolean($this, 'disabled', $val);
