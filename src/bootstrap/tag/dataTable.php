...
class DataTable extends \Lucid\Html\Bootstrap\Tag\Table
{
    public $id           = null;
    public $data         = null;
    ...
    public function init()
    {
        ...
        $this->topleft     = \DevLucid\html::span()->pull('left')->style('width:1%;');
        $this->topright    = \DevLucid\html::span()->pull('right')->style('width:1%;');
        $this->bottomleft  = \DevLucid\html::span()->pull('left');
        $this->bottomright = \DevLucid\html::span()->pull('right');
    }
        ...
        foreach($this->components as $component_name=>$component) {
            if (is_null($component) === false) {
                ...
                if (is_object($component) === false) {
                    $component = \DevLucid\html::span($component);
                }
    ...
    protected function retrieveData()
    {
        ...
        $func = \DevLucid\html::$hooks['data_table__query'];
    ...
    {
        $header = \DevLucid\html::cardHeader();
    ...
    {
        $footer = \DevLucid\html::cardFooter();
    ...
    {
        $row = \DevLucid\html::row();
        $row->add(\DevLucid\html::td()->colspan(count($this->children)));
        $row->lastChild()->add(\DevLucid\html::alert('warning', $this->msgDataNoFoundTitle, $this->msgDataNotFoundBody));
        return $row->render();
    }

    public function buildPager()
    {
        $pager = \DevLucid\html::buttonGroup(); #->size('sm');

        $pager->add(\DevLucid\html::button(\DevLucid\html::icon('fast-backward'), 'primary', 'html.dataTable.page(this,\'first\');'));
        $pager->add(\DevLucid\html::button(\DevLucid\html::icon('backward'), 'primary', 'html.dataTable.page(this,\'previous\');'));

        $pager->add(\DevLucid\html::select(null, $this->currentPage, $this->buildPagerOptions(), "html.dataTable.page(this,this.options[this.selectedIndex].value);"));
        $pager->lastChild()->modifier('primary');

        $pager->add(\DevLucid\html::button(\DevLucid\html::icon('forward'), 'primary', 'html.dataTable.page(this,\'next\');'));
        $pager->add(\DevLucid\html::button(\DevLucid\html::icon('fast-forward'), 'primary', 'html.dataTable.page(this,\'last\');'));
    ...
    public function enableSelectFilter($field, $data)
    {
        ...
        $this->components['filter_select'] = \DevLucid\html::select(null, $value, $data, "html.dataTable.filter(this,false);")
            ->id($id)
            ->size('sm')
            ->style('width:auto;');
    ...
    public function enableSearchFilter($fields, $filter_name = 'search')
    {
        ...
        $this->components['filter_search'] = \DevLucid\html::input('text', $filter_name, $value)
            ->id($id)
            ->preAddon(\DevLucid\html::icon('search'))
            ->size('sm')
            ->style('width:auto;')
    ...
    {
        $this->components['add_new'] = \DevLucid\html::anchorButton($url, $label, 'primary');
        return $this;
    }
}

if (isset(\DevLucid\html::$hooks['data_table__query']) === false || is_null(\DevLucid\html::$hooks['data_table__query']) === true) {
    \DevLucid\html::$hooks['data_table__query'] = function($data, $column = null, $direction = null, $page = 0, $limit = 10, $filters = []) {
        # apply filters
